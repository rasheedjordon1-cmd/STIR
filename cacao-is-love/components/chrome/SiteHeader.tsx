      ...
      <header className={s.header}>
        <div className={`shell ${s.headerInner}`}>
          ...
          <div className={s.headerRight}>
            ...
            >
              BAG
              <span className={`${s.cartCount} ${count === 0 ? s.cartCountZero : ''}`}>
                {count}
              </span>
            </button>
            <button
      ...
      {menuOpen && (
        <div ref={sheetRef} className={s.sheet} id="mobile-menu" role="dialog" aria-modal="true" aria-label="Menu">
          ...
          <nav className={s.sheetNav} aria-label="Primary">
            {[...site.nav, { label: 'SHOP', href: site.shopHref }].map((item, i) => (
              <Link
                key={item.label}
                href={item.href}
                ...
                onClick={() => setMenuOpen(false)}
              >
                <span className={`t-meta ${s.sheetNum}`}>{String(i + 1).padStart(2, '0')}</span>
                {item.label}
              </Link>
            ))}
          </nav>
