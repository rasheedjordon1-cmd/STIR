...
| M-17 | Row Disclosure | hover or focus on a compliance row, desktop | 140 in, 240 out | The cold layer slides under the row, an Amber rule draws down its left edge, and a second line appears naming what that row produces on the paperwork. Touch screens and reduced motion show the line at rest | What travels with the load for each commitment |
| M-18 | Reading Line | touch screens only, while the section is in view | 1,600 ms per item, loops | A reading line walks the product and retail lists one item at a time; each item answers with its zone or its step in the building. Hover does the same on desktop, so nothing loops there. Reduced motion shows every answer at rest | Which zone each product lives in, and which step each retail job touches |
